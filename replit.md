...
## Complete PVP Combat System & Menu Redesign (October 12, 2025)

### Card Spawning System
- **Disabled Auto-Spawning**: Removed automatic card spawning from bot startup
- **Manual Card Spawn**: Created `/spawncard` command with 5-minute cooldown per user
- **User-Controlled**: Cards now only spawn when users manually trigger the command

### Menu System Overhaul
- **Simplified Categories**: Menu now shows ONLY Economy and Cards categories
- **Economy Subcategories**: 
  - 💰 Bank & Money (balance, bank, deposit, withdraw, daily, give)
  - 🎰 Games & Gambling (slot, aviator, gamble, dice)
  - 💼 Jobs & Business (job, work, business, property)
  - 💸 Investments (invest, stock, crypto)
  - 🚔 Crime & Jail (crime, bail, paycops, reportattack)
  - 🛒 Shopping (shop, pharmacy, blackmarket)
  - 📊 Stats & Progress (stats, inventory, leaderboard, missions)
  - ⚔️ PVP Combat (challenge, accept, decline, pvpattack, punch, dodge, weave, block)
  - 💳 Loans & Credits (loan, loanform, repay)
  - 🍺 Bar & Effects (bar, barstats)

### PVP Combat System
- **Weapons in Shop**: Added 14+ combat weapons with damage stats
  - 🔫 Pistol (10-20 dmg, 80% accuracy)
  - 🔫 Shotgun (25-40 dmg, 60% accuracy)
  - 🔫 Rifle (20-35 dmg, 75% accuracy)
  - 🎯 Sniper (40-70 dmg, 50% accuracy)
  - 🔪 Knife (8-15 dmg, 90% accuracy)
  - 🗡️ Katana (15-30 dmg, 75% accuracy)
  - 🪓 Machete (12-25 dmg, 70% accuracy)
  - ⚾ Baseball Bat (10-20 dmg, 85% accuracy)
  - 🍾 Bottle (5-15 dmg, 80% accuracy, 30% stun)
  - ⚡ Taser (3-8 dmg, 90% accuracy, 50% stun)
  - 💣 Grenade (30-60 dmg, 70% accuracy, explosive)
  - 🛡️ Body Armor (30% damage reduction)
  - 🥊 Boxing Gloves (+5 punch damage)
  - 💊 Med Kit (restores 50 HP)

- **Challenge System**: 
  - `/challenge @user` or reply to message with `/challenge`
  - Opponent has 60 seconds to accept or decline
  - Both players must have HP > 30 and not be in jail
  - Challenge key format: `${challengerId}_${targetId}_${chatId}` (prevents duplicates)

- **Turn-Based Combat**:
  - Challenger starts first, alternating turns
  - Players can attack with weapons or use combat moves
  - Combat commands: `/pvpattack <weapon>`, `/punch`, `/dodge`, `/weave`, `/block`
  - Each move deducts HP based on damage and accuracy
  - Battle ends when one player's HP reaches 0

- **Health System**:
  - All users start with 100 HP
  - HP shown in `/balance` command with visual bar
  - Damage persists after battles
  - Heal using `/pharmacy` or `/bar` commands

- **PVP Stats Tracking**:
  - Win/loss records saved to database
  - PVP stats displayed in `/balance` command
  - Persistent across all battles

### Technical Implementation
- **Shared State**: Exported `activeChallenges` and `activeBattles` Maps from challenge.js
- **Proper Key Management**: Fixed challenge key mismatch for duplicate prevention
- **Database Integration**: All combat stats persist in MongoDB
- **Command Loading**: 476 total commands loaded successfully
