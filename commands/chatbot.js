...
export default {
    ...
    async execute(msg, { sock, bot, args }) {
        ...
        const isGroup = from.endsWith('@g.us') || (bot && !isNaN(parseInt(from)));
        ...
        if (bot) {
            ...
            if (chat.type !== 'group' && chat.type !== 'supergroup') {
                return await bot.sendMessage(chatId, '❌ This command can only be used in groups.');
            }
            
            const command = args[0]?.toLowerCase();
            if (!command) {
                return await bot.sendMessage(chatId, `*🤖 HORLAPOOKIE CHATBOT SETUP*\n\n/chatbot on - Enable chatbot\n/chatbot off - Disable chatbot\n\n*Note:* Only admins can use this command.`, { parse_mode: 'Markdown' });
            }
            ...
            if (command === 'on') {
                if (data.chatbot[from]) {
                    return await bot.sendMessage(chatId, '✅ Chatbot is already enabled for this group.');
                }
                data.chatbot[from] = true;
                saveUserGroupData(data);
                return await bot.sendMessage(chatId, '🤖 *Horlapookie Chatbot has been enabled!*\n\nMention me or reply to my messages to chat with me. 😊', { parse_mode: 'Markdown' });
            }
            
            if (command === 'off') {
                if (!data.chatbot[from]) {
                    return await bot.sendMessage(chatId, '❌ Chatbot is already disabled for this group.');
                }
                delete data.chatbot[from];
                saveUserGroupData(data);
                return await bot.sendMessage(chatId, '🤖 *Horlapookie Chatbot has been disabled.*', { parse_mode: 'Markdown' });
            }
            
            return await bot.sendMessage(chatId, '❌ Invalid command. Use `on` or `off`.', { parse_mode: 'Markdown' });
        }
...
export async function handleChatbotResponse(sock, msg, userMessage, senderId, bot = null) {
    const from = msg.key.remoteJid;
    const data = loadUserGroupData();
    ...
    try {
        ...
        if (bot) {
            ...
            if (msg.message?.reply_to_message?.from?.id === botInfo.id) {
                isReplyToBot = true;
            }
            ...
            // Clean message
            let cleanedMessage = userMessage.replace(new RegExp(`@${botUsername}`, 'g'), '').trim();
            ...
            if (!response) {
                await bot.sendMessage(chatId, toUnicodeMath("Hmm, let me think about that... 🤔\nI'm having trouble processing your request right now."));
                return;
            }
            
            const unicodeResponse = toUnicodeMath(response);
            await bot.sendMessage(chatId, unicodeResponse);
            return;
        }
