...
export default {
  ...
  async execute(msg, { sock, bot, args, settings }) {
    ...
    // Telegram handling
    if (bot) {
      const replyMsg = msg.message?.reply_to_message || msg.key?.quotedMessage;
      
      if (!replyMsg) {
        return await bot.sendMessage(parseInt(from), '❌ Please reply to an image, video, or audio message to upload to catbox.moe!');
      }

      try {
        await bot.sendMessage(parseInt(from), '📤 Uploading media to catbox.moe... Please wait!');
        ...
        if (replyMsg.photo && replyMsg.photo.length > 0) {
          ...
          mimeType = replyMsg.document.mime_type || 'application/octet-stream';
          fileExtension = path.extname(replyMsg.document.file_name || '') || '.bin';
        }

        if (!fileId) {
          return await bot.sendMessage(parseInt(from), '❌ No supported media found in the reply message.');
        }

        // Download media using Telegram API
        ...
        if (!buffer || buffer.length === 0) {
          return await bot.sendMessage(parseInt(from), '❌ Failed to download the media. Please try again.');
        }
        ...
        if (response.data && response.data.startsWith('https://files.catbox.moe/')) {
          ...
          await bot.sendMessage(parseInt(from), 
        ...
        console.error('Catbox Telegram upload error:', error);
        await bot.sendMessage(parseInt(from), `❌ Upload failed: ${error.message}`);
      }
      return;
    }
