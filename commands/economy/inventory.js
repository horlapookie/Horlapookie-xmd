  ...
  async execute(msg, { bot }) {
    ...
    try {
      ...
      let stats = `💰 *Your Economy Stats*\n\n` +
        ...
        `💵 Balance: ${escapeMarkdown(user.balance.toLocaleString())} coins\n` +
        `🏦 Bank: ${escapeMarkdown((user.bank || 0).toLocaleString())}/${escapeMarkdown((user.bankLimit || 10000).toLocaleString())} coins\n` +
        `📈 Total Earned: ${escapeMarkdown(user.totalEarned.toLocaleString())} coins\n` +
        `📉 Total Spent: ${escapeMarkdown(user.totalSpent.toLocaleString())} coins\n\n` +
        `*🏥 HEALTH & LIFESTYLE*\n` +
