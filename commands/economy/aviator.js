...
export default {
  ...
  async execute(msg, { bot, args }) {
    ...
    try {
      ...
      // Animate the aviator (much slower and more stable to prevent glitching)
      while (multiplier < crashPoint && multiplier < 15 && !cashedOut) {
        multiplier += 0.03; // Smaller increment for smoother animation
        global.aviatorGames[userId].multiplier = multiplier;
        
        // Only update message every 800ms to avoid rate limits and prevent glitching
        const now = Date.now();
        ...
        await new Promise(resolve => setTimeout(resolve, 400)); // Increased from 250ms to 400ms
      }
